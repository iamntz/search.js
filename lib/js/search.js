...
;var VisualSearch = (function($, _){
	...
	$.widget("custom.visualsearchAutocomplete", $.ui.autocomplete, {
		_create: function() {
			this._super();
	...
	VS.ParameterView = Backbone.View.extend({
		...
		inputFocused: function(e){
			$(e.target).visualsearchAutocomplete("search", "");
		},
		...
		autocomplete: function(target, options){
			target
				.visualsearchAutocomplete(options)
				.visualsearchAutocomplete('widget')
				.addClass('VS-interface');
			return target;
		},
		keydown : function(e) {
