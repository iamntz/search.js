...
;var VisualSearch = (function($, _){
	...
	$.widget("ui.autocomplete", $.ui.autocomplete, {
		_renderMenu: function( ul, items ) {
			...
			$.each( items, function( index, item ) {
				...
				if(item.category && item.category == currentCategory){
					dom.children().css({'padding-left': '14px', 'font-size':'12px'});
				}
			});
		}
	});
	...
	var VS = Backbone.View.extend({
		...
		initialize : function(){
			...
			this.options =	_.extend({
				el	: '',
				defaultquery: [],
				placeholder	: "Hello I am the default placeholder",
				strict: true,	//Only accept parameters that are defined
				search: $.noop
			}, this.options);

			//Organize Labels into Categories
			this.parameters = _.transpose(this.options.parameters);
			for( var i in this.parameters.parameter){
				this.parameters.category[i] = { label: this.parameters.parameter[i], category: this.parameters.category[i]}
			}
			...
			.on({
				"add remove change:editing": function(m){
					if(m.collection){
						self.options.search(JSON.stringify(m.collection.getComplete()));
					}
				},
				"all": function(e, m){
					self.render();
			...
			if (this.searchQuery.length) {
				...
				$('.VS-search-inner', template).append(this.searchQuery.map(function(parameter){
					var parameterView = new VS.ParameterView({
						model : parameter
					}).render(self.parameters, self.options.strict).el;
		...
		highlightSearch: function(e){
			if(!$(e.target).is(".VS-search-box")) return;
	...
	VS.template = 
	{
		...
		'search_parameter': _.template('<div class="search_parameter_remove VS-icon VS-icon-cancel"></div><div class="parameter"><%- model.get(\'parameter\') %></div><div class="operator"><%- model.get(\'operator\') %></div><div class="value"><%- model.get(\'value\') %></div>')
	};
	...
	VS.ParameterView = Backbone.View.extend({
		...
		render : function(parameters, strict) {
			...
			this.parameter = {
				dom: $("<input/>").attr({
					autocomplete:	"off",
					name:			"parameter",
					value: this.model.get("parameter"),
					width: '10px'
				}),
				autocomplete: {
					minLength : 0,
			...
			this.operator = {
				...
				autocomplete: {
					...
					source: function(req, res){
						var parameter 	= self.model.get('parameter'),
							i = parameters.parameter.indexOf(parameter);
			...
			if( !this.model.has('parameter') || this.model.get('editing')===0 ){
				this.autocomplete(
					this.$("div.parameter").html(this.parameter.dom).children(),
					this.parameter.autocomplete
				).focus(0);
			}else{
		...
		inputBlurred: function(e){
			...
			if( this.model.get(input.attr("name"))==input.val() && editing!=1 ){
				this.model.set('editing', null);
			}else

			//Update Value and go to next field
			{
				var update = {
				...
				if(!editing){
					var self = this,
						i = this.parameters.parameter.indexOf(input.val());

					//Kill if the parameter doesn't exist?
					if(this.strict && i === -1 ){ return this.model.destroy(); }

					_.extend(update, {
						"type": self.parameters.type[i],
						"placeholder": self.parameters.placeholder[i],
						"min": self.parameters.min[i],
						"max": self.parameters.max[i]
					});
				}
		...
		keydown : function(e) {
			...
			if(
				e.keyCode===8 &&
				$(e.target).getCursorPosition()===0 &&
				$(e.target).getSelection().length===0
			){
				this.model.destroy();
			}
		}
	...
	VS.Parameter = Backbone.Model.extend({
		...
		//Default Parameters
		defaults: {
			parameter: null,	//Name of Parameter
		...
		incomplete: function(){
			return !(this.has('parameter') && this.has('operator') && this.has('value'));
		}
